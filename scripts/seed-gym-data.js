  ...
  try {
    ...
    for (const gym of gymData) {
      try {
        ...
        console.log(`✅ 成功创建场馆: ${gym.name} (ID: ${result.id})`);
      } catch (error) {
    ...
    const beijingGyms = await prisma.gym.count({ where: { city: "北京" } });
    const shanghaiGyms = await prisma.gym.count({ where: { city: "上海" } });
    const crossfitCertified = await prisma.gym.count({ where: { crossfitCertified: true } });
    
    console.log('\n📊 数据库统计:');
    console.log(`总场馆数: ${totalGyms}`);
    console.log(`北京场馆: ${beijingGyms}`);
    console.log(`上海场馆: ${shanghaiGyms}`);
    console.log(`CrossFit认证场馆: ${crossfitCertified}`);
